    ...
    <div className="space-y-12 font-sans">
      ...
      <div className="relative">
        <div className="space-y-1">
          <h2 className="text-5xl font-bold tracking-tight text-text-primary serif-heading">Warranty Portfolio.</h2>
          <p className="text-lg text-text-secondary font-medium opacity-80 italic">List of verified product warranties and coverage details.</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-8">
         {isLoading ? (
           ...
           <div key={p.Product_ID} className="bg-bg-primary p-10 relative group overflow-hidden border border-border-color rounded-[3rem] shadow-sm hover:shadow-[0_15px_45px_-10px_rgba(0,0,0,0.08)] transition-all">
              <div className={`absolute top-0 left-0 w-1.5 h-full ${p.IsUnderWarranty ? 'bg-black' : 'bg-border-color'}`}></div>
              <div className="flex justify-between items-start mb-10">
                 <div className="bg-bg-secondary p-4 rounded-2xl border border-border-color">
                    <ShieldCheck className="w-6 h-6 text-text-primary opacity-40" />
                 </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 mt-12">
         ...
         <div className="lg:col-span-1 space-y-8">
             <div className="bg-black p-12 text-white rounded-[3.5rem] shadow-2xl relative group overflow-hidden">
               <h4 className="text-3xl font-bold tracking-tight mb-6 italic uppercase serif-heading leading-tight">Elevate your <br/>protection.</h4>
               <p className="text-white/60 text-sm font-medium leading-relaxed mb-12 italic">Upgrade to Enterprise Ultra for accidental coverage and instant on-site technician deployment across all global hubs.</p>
               <button 
                 onClick={() => navigate('/warranty-plans')}
                 className="w-full bg-white text-black py-6 rounded-full font-bold text-[12px] uppercase tracking-[.4em] hover:scale-[1.02] active:scale-95 transition-all border-none cursor-pointer shadow-lg"
               >
                  Upgrade Plan
               </button>
            </div>

            <div className="bg-bg-secondary/30 p-12 border border-border-color rounded-[3.5rem] shadow-sm">
               <h5 className="text-[11px] font-bold uppercase tracking-[.4em] text-text-secondary opacity-40 mb-10 italic">Verification Hub</h5>
               <div className="flex flex-col items-center">
                  <div className="w-16 h-16 rounded-2xl bg-bg-secondary border border-border-color flex items-center justify-center text-text-primary mb-6 shadow-sm">
                     <ShieldCheck className="w-8 h-8 opacity-40" />
                  </div>
                  <div className="text-[15px] font-bold text-text-primary italic serif-heading tracking-tight">Authenticated Sync</div>
                  <div className="text-[10px] font-bold text-text-secondary uppercase tracking-[.4em] opacity-30 mt-2">SEC-9921</div>
               </div>
            </div>
         </div>
      </div>

  ...
  return (
    <div className="space-y-12 font-sans">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
        {stats.map((stat, i) => (
          <div key={i} className="bg-bg-secondary p-10 group relative overflow-hidden border border-border-color rounded-[2.5rem] transition-all hover:bg-white hover:shadow-sm">
             <div className="absolute top-0 right-0 p-8 opacity-[0.03] group-hover:opacity-[0.08] transition-opacity">
               <stat.icon className="w-24 h-24 text-black" />
             </div>
             ...
             <div className="text-6xl font-bold tracking-tighter text-black leading-none mb-3 italic">{stat.val}</div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-8">
        {/* Activity Feed */}
        <div className="lg:col-span-3 bg-white p-10 border border-border-color rounded-[2.5rem] shadow-sm">
           <div className="flex justify-between items-center mb-10 text-center md:text-left">
              <div>
                <h4 className="text-xl font-bold tracking-tight text-black italic serif-heading">Real-Time Registry</h4>
              ...
              {recentActivity.length === 0 ? (
                ...
                <div key={act.id} className="flex gap-6 group hover:translate-x-2 transition-all duration-300">
                   <div className="w-14 h-14 rounded-2xl bg-bg-secondary border border-border-color flex items-center justify-center shrink-0 transition-all">
                      <Box className="w-6 h-6 text-black opacity-20 group-hover:opacity-100" />
                   </div>
                   <div className="flex-1 flex flex-col justify-center">
                      <div className="flex justify-between items-start">
                        <h5 className="text-[15px] font-bold text-black tracking-tight italic">{act.asset}</h5>
                      ...
                      <p className="text-[12px] font-medium text-text-secondary mt-1.5 opacity-80 leading-none">
                        Protocol <span className="text-black font-mono font-bold">#{act.protocol}</span> status: <span className={`font-bold tracking-widest uppercase ${act.status === 'Completed' ? 'text-black' : 'text-text-secondary'}`}>{act.status}</span>.
                      </p>
                   </div>
                </div>
              ))}
        ...
        <div className="lg:col-span-2 bg-bg-secondary p-10 border border-border-color rounded-[2.5rem] flex flex-col items-center">
           <div className="w-full mb-10">
             <h4 className="text-xl font-bold tracking-tight text-black italic serif-heading">Health Matrix</h4>
           ...
           <div className="relative w-56 h-56 mb-12">
              ...
              <div className="absolute inset-0 flex flex-col items-center justify-center animate-in">
                 <span className="text-5xl font-bold text-black tracking-tighter leading-none italic italic serif-heading">94<span className="text-2xl text-text-secondary">.2%</span></span>
                 <span className="text-[10px] uppercase font-bold text-black tracking-[0.2em] mt-3">ENFORCED</span>
              </div>
           </div>
           ...
           <div className="w-full space-y-4">
              {[
                ...
                <div key={i} className="flex justify-between items-center px-5 py-4 bg-white rounded-2xl border border-border-color">
                   ...
                   <span className="text-[11px] font-bold text-black font-mono">{item.val}</span>
                </div>
              ))}
           </div>
        </div>
